...
class XmlMapperBuilderTest {
  ...
  void shouldSuccessfullyLoadXMLMapperFile() throws Exception {
    ...
    String resource = "org/apache/ibatis/a2_核心处理层/builder_配置解析过程/AuthorMapper.xml";
  ...
  void shouldFailedLoadXMLMapperFile() throws Exception {
    Configuration configuration = new Configuration();
    String resource = "org/apache/ibatis/a2_核心处理层/builder_配置解析过程/ProblemMapper.xml";
    try (InputStream inputStream = Resources.getResourceAsStream(resource)) {
      XMLMapperBuilder builder = new XMLMapperBuilder(inputStream, configuration, resource, configuration.getSqlFragments());
...
//  }
   ...
   public void erorrResultMapLocation() throws Exception {
     Configuration configuration = new Configuration();
     String resource = "org/apache/ibatis/a2_核心处理层/builder_配置解析过程/ProblemResultMapper.xml";
     try (InputStream inputStream = Resources.getResourceAsStream(resource)) {
       XMLMapperBuilder builder = new XMLMapperBuilder(inputStream, configuration, resource, configuration.getSqlFragments());
       builder.parse();
